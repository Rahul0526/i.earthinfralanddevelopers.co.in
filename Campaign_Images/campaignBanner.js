function LoadAd(IsView, AdLoad) {
    var EncryptedString = document.getElementById('DivControl').textContent; //"U2FsdGVkX1+V2mTAHFhWNXUHmwtg6qgaWQn0Q71KPZIyKHKziXRoIe96ykP8HuSJXnE0qnqiByCK5Kw0A03jKC9ha+yhCVm8e1lKFZapTZ6m+yDgwWiFWUsShFLADs7oLCDVAOrXy+xh6pA3w1L7hi7NCihSaDGJ4b/jrccdnrkpIAOjXOSVfVJTD6XeVF/VmdPFKsxNYZdwC6iVjsyv8g==";
    var PasteString = CryptoJS.AES.decrypt(EncryptedString, "AUGURS001").toString(CryptoJS.enc.Utf8);
    document.getElementById('DivControl').style.display = "none"; //Hide DivControl
    console.log("Output;");
    console.log(location.hostname);
    console.log(document.domain);
    var domain_name = document.domain;
    var AdPageUrl = window.location.href;
    
    var Arr = PasteString.toString().split(';');

    var websites = Arr[0]; //document.getElementById('ContentPlaceHolder1_sp_All_sites_info').innerText;
    ...
    if (OSName != "Unknown OS") {
        ...
        {
            var req_type = "view";
            var Affiliate_id = Arr[1].toString(); //document.getElementById('ContentPlaceHolder1_sp_Affiliate_id').innerText;
            var banner_id = Arr[2].toString(); //document.getElementById('ContentPlaceHolder1_sp_span_banner_id').innerText;
            var os_type = OSName;
            var bg_color = Arr[3].toString(); //document.getElementById('ContentPlaceHolder1_sp_ad_bg_color').innerText;
            var button_color = Arr[4].toString(); //document.getElementById('ContentPlaceHolder1_sp_ad_button_color').innerText;
            ...
            $.getJSON("http://freegeoip.net/json/", function (data) {
                ...
                var ip = data.ip;
                var url = "http://camp.earthinfralanddevelopers.co.in/(S(0casoi4cnyzmum00r23kv2fn))/admin/Campaign_Ad_Panel_View.aspx?camp_id=" + camp_id + "&request_type=" + req_type + "&affiliate_id=" + Affiliate_id + "&domain_name=" + domain_name + "&bg_color=" + bg_color + "&button_color=" + button_color + "&banner_id=" + banner_id + "&os_type=" + os_type + "&country_name=" + country + "&View=" + IsView + "&AdToLoad=" + AdToLoad + "&NoOfBanner1=" + total_banner1 + "&Banner2BorderColor=" + Banner2BorderColor + "&banner2TextColor=" + Banner2textColor + "&AdPageUrl=" + AdPageUrl;
                ...
                if (banner_id != 'Banner 1') {
                    $.ajax({ url: url, success: function (response) {
                        ...
                        if (IsView == 0) {
                            $result = $(response).find("#randomcampid").html();
                            url1 = "http://camp.earthinfralanddevelopers.co.in/(S(0casoi4cnyzmum00r23kv2fn))/admin/Campaign_Ad_Panel_View.aspx?camp_id=" + $result + "&request_type=" + req_type + "&affiliate_id=" + Affiliate_id + "&domain_name=" + domain_name + "&bg_color=" + bg_color + "&button_color=" + button_color + "&banner_id=" + banner_id + "&os_type=" + os_type + "&country_name=" + country + "&View=" + IsView + "&AdToLoad=" + AdToLoad + "&NoOfBanner1=" + total_banner1 + "&Banner2BorderColor=" + Banner2BorderColor + "&banner2TextColor=" + Banner2textColor + "&AdPageUrl=" + AdPageUrl;
                           
                            var ifrm = '<iframe frameborder="0" id="my_frame" class="frame" src="' + url1 + '" scrolling="no" onload="resizeIframe(\'my_frame\');"></iframe>';
                            $("#DivControl").after(ifrm)
                        }
                        else if (IsView == 1 && typeof $result === 'undefined') {
                            $result = $(response).find("#randomcampid").html();
                            url1 = "http://camp.earthinfralanddevelopers.co.in/(S(0casoi4cnyzmum00r23kv2fn))/admin/Campaign_Ad_Panel_View.aspx?camp_id=" + $result + "&request_type=" + req_type + "&affiliate_id=" + Affiliate_id + "&domain_name=" + domain_name + "&bg_color=" + bg_color + "&button_color=" + button_color + "&banner_id=" + banner_id + "&os_type=" + os_type + "&country_name=" + country + "&View=" + IsView + "&AdToLoad=" + AdToLoad + "&NoOfBanner1=" + total_banner1 + "&Banner2BorderColor=" + Banner2BorderColor + "&banner2TextColor=" + Banner2textColor + "&AdPageUrl=" + AdPageUrl;
                        ...
                        if (IsView == 1) {
                            $.ajax({ url: url1, success: function (response) {
                            }
                            });
                        }
                        else {
                    ...
                    var ifrm = '<iframe frameborder="0" id="my_frame" class="frame" src="' + url + '" scrolling="no" onload="resizeIframe(\'my_frame\');"></iframe>';
                    $("#DivControl").after(ifrm)
                }
            });
        }
