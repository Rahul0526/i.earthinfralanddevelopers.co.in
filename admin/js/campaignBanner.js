...
function initJQuery() {
    if (typeof(jQuery) == 'undefined') {
        ...
        if (!jQueryScriptOutputted) {
            ...
            alert("jquery not included. Please include jquery before campaignBanner.js")
                //            document.head.insertBefore(document.getElementsByTagName('script')[0], '<script language="javascript" src="http://i.earthinfralanddevelopers.co.in/admin/js/jquery-1.11.3.min.js" TYPE="text/javascript"></SCRIPT>');
        }
        // document.write('<script language="javascript" src="http://www.google.com/jsapi?key=YOURKEY" TYPE="text/javascript"></SCRIPT>');
    ...
    $(function () {
        ...
        function LoadAd(IsView, EncryptedString, targetElem) {
            ...
            var AdPageUrl = window.location;
            var Arr = PasteString.toString().split(';');
            ...
            if (navigator.appVersion.indexOf("iPhone") != -1) OSName = "Apple";
            if (navigator.appVersion.indexOf("iPod") != -1) OSName = "iPod";
            if (navigator.appVersion.indexOf("iPad") != -1) OSName = "iPad";
            if (OSName != "Unknown OS") {
                var req_type = "view";
                var Affiliate_id = Arr[1].toString();
                var banner_id = Arr[2].toString();
                var os_type = OSName;
                var bg_color = Arr[3].toString();
                var button_color = Arr[4].toString();
                var total_banner1 = Arr[5].toString();
                var Banner2BorderColor = "no color";
                var Banner2textColor = "no color";

                if (banner_id == 'Banner 2') {
                    Banner2BorderColor = Arr[6].toString();
                    Banner2textColor = Arr[7].toString();
                }
                ...
                if (banner_id == 'Banner 5') {
                    TitleColor = "no color";
                    DescriptionColor = "no color";
                    BorderColor = "no color";
                    ShowCallToAct = "Show";
                    showImage = "Show";
                } else {
                    TitleColor = Arr[8].toString();
                    DescriptionColor = Arr[9].toString();
                    BorderColor = Arr[10].toString();
                    PageBackColor = Arr[11].toString();
                    ShowCallToAct = Arr[12].toString();
                    showImage = Arr[13].toString();
                    PassBackId = Arr[14].toString();
                }
                var country = "";
                var isp = "";
                $.ajax({
                    url: 'http://35.154.123.137/ipserver/getIpData.php?auth=n98fghSDS67JOu6cE',
                    type: 'GET',
                    dataType: "json",
                                    ...
                                    if (!(result.indexOf('alert(') >= 0)) {
                                        ...
                                        if (result != "" || banner_id == "Banner 1" || banner_id == "Banner 4") {
                                            if (banner_id == 'Banner 1') {
                                                if (IsView != 0) {
                                                    url1 = domain + "Admin/Campaign_Ad_Panel_View.aspx?camp_id=" + result + "&request_type=" + req_type + "&affiliate_id=" + Affiliate_id + "&domain_name=" + domain_name + "&bg_color=" + bg_color + "&button_color=" + button_color + "&banner_id=" + banner_id + "&os_type=" + os_type + "&country_name=" + country + "&isp=" + isp + "&View=" + IsView + "&AdToLoad=" + AdToLoad + "&NoOfBanner1=" + total_banner1 + "&Banner2BorderColor=" + Banner2BorderColor + "&banner2TextColor=" + Banner2textColor + "&AdPageUrl=" + AdPageUrl + "&Country=" + country + "&TitleColor=" + TitleColor + "&DescriptionColor=" + DescriptionColor + "&BorderColor=" + BorderColor + "&PageBackColor=" + PageBackColor + "&ShowCallToAct=" + ShowCallToAct + "&showImage=" + showImage + "&PassBackId=" + PassBackId;
                                                }
                                                ...
                                                ifrm = '<iframe frameborder="0" id="my_frame" data-frame-id="' + AddNo + '" data-view="' + IsView + '" height="0" class="frame" src="' + url1 + '" scrolling="no" data-campId="' + result + '" data-bannerId="' + banner_id + '" data-affiliateId="' + Affiliate_id + '" data-domain="' + domain_name + '" data-os="' + os_type + '" data-country="' + country + '"></iframe>';
                                                targetElem.after(ifrm)
                                            }
                                        }
                                    }
                                    /*else {
                                    console.log("Server error caused");
                                    }*/
                                }
                            });
                        }
        ...
        $(window).load(function () {
            if ($('div[id=DivControl]').length) { $('div[id=DivControl]').innerHeight(2) }
            LoadByScroll = 1;
            $('div[id=DivControl]').each(function () {
                ...
                encryptedString = $(this).text();
                LoadAd(ViewCount, encryptedString, $(this));
            })
        });
                ...
                $("[id=my_frame]").each(function () {
                    if (parseInt($(this).attr('data-view')) == 2) {
                        if ($(this).isOnScreen(0.5, 0.5) == true) {
                            ...
                            parameter = JSON.stringify({ "campId": campId, "bannerId": bannerId, "affiliateId": affiliateId, "domain": domain_name, "os": os, "country": country });
                            var pageUrl = domain + "admin/updateAdViews.aspx/updateView";
                            $.ajax({
                                type: 'POST',
                                url: pageUrl,
                                data: parameter,
...
function resizeIframe(obj) {
    try {
        var framewidth = $("#my_frame").contents().width();
        var pwidth = $('#my_frame').parent().width();
        if (framewidth > pwidth) { framewidth = pwidth - 10; }
        setTimeout(function() {}, 8000);
        document.getElementById(obj).style.height = $("#my_frame").contents().find('body').height() + 25 + 'px';
        document.getElementById(obj).style.width = pwidth + 'px';
    } catch (err) {
